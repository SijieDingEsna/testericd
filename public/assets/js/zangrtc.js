/*! zangrtc - v0.0.1 - 2016-06-16
 * Copyright (c) 2016 ; Licensed  */
var FSRTC = function(options) {
	this.options = $.extend({
		useVideo: null,
		useStereo: false,
		userData: null,
...
(function($) {
	...
	FSRTC.prototype.stopPeer = function() {
		if (this.peer) {
			console.log("FSRTC stopping peer");
			this.peer.stop();
		}
	}
...
var VERTO = function(options, callbacks) {
		var verto = this;
...
var ZangRTC = {
	hostname: "webrtcmedia.zangcloud.com",
	wsUrl: "wss://webrtcmedia.zangcloud.com:8082"
};


ZangRTC.ENUM = function(s) {
	var i = 0,
		o = {};
	s.split(" ").map(function(x) {
		o[x] = x;
...
ZangRTC.events = ZangRTC.ENUM("login disconnected message ringing answer hangup trying");

ZangRTC.client = function(sid, password, nickname, options) {
	this.sid = sid;
	...
	this.callbacks = {};
	this.options = $.extend({
		ringFile: "demo/sounds/bell_ring2.wav",
		stereo: false,
		video: true,
		// ice: true,
		ice: [
			{
				url: "stun:104.236.42.94:3478"
			},
			{
				url: "turn:104.236.42.94:3478"
			}
		],
		videoParams: {
			"minWidth": "1280",
...
ZangRTC.client.prototype.connect = function() {
	this.verto = new VERTO({
		login: this.sid + "@" + this.options.hostname,
		passwd: this.password,
		socketUrl: ZangRTC.wsUrl,
		tag: "webcam",
		ringFile: this.options.ringFile,
		videoParams: this.options.videoParams,
		audioParams: this.options.audioParams,
		iceServers: this.options.ice
	}, this.getCallbacks());
};

ZangRTC.client.prototype.on = function(event, fn) {
	this.callbacks[event] = fn;
};

ZangRTC.client.prototype.doCallback = function(event, a, b, c, d, e, f, g, h, i, j, k, l) {
	var cb = this.callbacks[event];
	if (typeof(cb) === "function") {
		cb(this, a, b, c, d, e, f, g, h, i, j, k, l);
	}
}

ZangRTC.client.prototype.getCallbacks = function() {
	var self = this;
	return {
		onWSLogin: function(v, success) {
			self.doCallback(ZangRTC.events.login, success);
			self.connected = success;
		},
		onWSClose: function(v, success) {
			// Connection closed
			self.connected = false;
			self.doCallback(ZangRTC.events.disconnected);
			// TODO: Callback
		},
					...
					if (data.pvtData) {
						...
						switch (data.pvtData.action) {
							...
							case "conference-liveArray-join":
								...
								console.log(data.pvtData.chatID);

								var conf = new VERTO.conf(verto, {
									dialog: dialog,
									hasVid: self.options.video,
									laData: pvtData,
									...
									onBroadcast: function(v, conf, message) {
										...
										if (message.action == 'response') {
											...
											if (message['conf-command'] == 'list-videoLayouts') {
												...
												console.log('conference.canvasInfo', message.responseData);
											} else {
												console.log('conference.broadcast', message);
											}
										}
									}
								});
								...
								// self.confMan = new VERTO.confMan(verto, {
								//     tableID: "#conf_list",
								//     statusID: "#conf_count",
								//     mainModID: "#conf_mod",
								...
								// self.liveArray = new VERTO.liveArray(
								//     verto, data.pvtData.laChannel,
								//     data.pvtData.laName, {
								//       subParams: {
								...
								// self.liveArray.onChange = function(obj, args) {
								//     console.log('liveArray.onChange', obj, args);

								//     switch (args.action) {
								//       case 'bootObj':
								//         console.log('members.boot', args.data);
								//         self.conferenceMembers = args.data;
								...
								//         console.log('members.add', member);
								//         break;
								//       case 'del':
								//         var uuid = args.key;
								//         console.log('members.del', uuid);
								//         break;
								//       case 'clear':
								//         console.log('members.clear');
								//         break;
								//       case 'modify':
								//         var member = [args.key, args.data];
							...
							default:
								console.log(data.pvtData);
						}
					}
					break;
				case VERTO.enum.message.info:
					// Got chat message
					self.doCallback(ZangRTC.events.message, data);
					break;
				case VERTO.enum.message.display:
					// Display (Talker?)
					var party = dialog.params.remote_caller_id_name + "<" + dialog.params.remote_caller_id_number + ">";
					console.log("Talking to: " + dialog.cidString());
					// TODO: Callback
					break;
				default:
					break;
		...
		onDialogState: function(d) {
			var cur_call = d;
			self.activeCall = new ZangRTC.call(this, d);
			...
			switch (d.state) {
				case VERTO.enum.state.ringing:
					// console.log("Incoming Call From: " + d.cidString());
					self.doCallback(ZangRTC.events.ringing, self.activeCall);
				...
				case VERTO.enum.state.trying:
					...
					self.doCallback(ZangRTC.events.trying, self.activeCall);
					break;
				// case VERTO.enum.state.early:
				case VERTO.enum.state.active:
					// Call answered
					console.log("Talking to: " + d.cidString());
					self.doCallback(ZangRTC.events.answer, self.activeCall);
					break;
				case VERTO.enum.state.hangup:
					// Call hung up
					console.log("Hung up");
					self.doCallback(ZangRTC.events.hangup, self.activeCall);
				case VERTO.enum.state.destroy:
...
ZangRTC.call = function(client, vcall) {
	this.client = client;
	this.vcall = vcall;
};

ZangRTC.client.prototype.call = function(options) {
	...
	options.destination_number = "client-" + this.sid + "-" + options.address;
	var settings = $.extend({
		caller_id_name: this.nickname,
		caller_id_number: this.sid,
		useVideo: this.options.video,
		useStereo: this.options.video
	}, options);


	var vcall = this.verto.newCall(settings);
	this.activeCall = new ZangRTC.call(this, vcall);
...
ZangRTC.call.prototype.hangup = function(options) {
	this.vcall.hangup();
}

ZangRTC.call.prototype.message = function(to, body) {
	var params = {
		to: to,
		body: body,
		from_msg_name: self.client.nickname,
		from_msg_number: self.client.sid
	};
	call.message(params);
}
